...
describe('Ternary Operator (??:)', () => {
    ...
    it('should parse ternary with operator function calls', () => {
        const result = parse('flag ?? +(a,b,c) ?: *(x,y,z)');
        const ast = result[0];
        
        expect(ast.type).toBe('TernaryOperation');
        expect(ast.trueExpression.type).toBe('FunctionCall');
        expect(ast.trueExpression.function.name).toBe('+');
        expect(ast.falseExpression.type).toBe('FunctionCall');
        expect(ast.falseExpression.function.name).toBe('*');
    });
