...
describe('Functional Control Structures', () => {
    ...
    test('mixed functional and traditional syntax tokens correctly', () => {
        ...
        const systemTokens = functionalTokens.filter(t => 
        ...
        const traditionalSystemTokens = traditionalTokens.filter(t => 
            t.type === 'Identifier' && t.kind === 'System'
        );
        expect(traditionalSystemTokens.length).toBe(2);
        expect(traditionalSystemTokens.map(t => t.value)).toEqual(['WHILE', 'DO']);
    });
    ...
    test('multiple control structures in sequence', () => {
        ...
        const code = 'sum := 0; WHILE(i < n, sum := sum + i; i := i + 1); result := IF(sum > 0, sum, 0)';
        const tokens = tokenize(code);
        const ast = parse(tokens, systemLoader.createParserLookup());
        ...
        const getNode = (stmt) => stmt.type === 'Statement' ? stmt.expression : stmt;
...
describe('Real-world functional control usage', () => {
    test('mathematical computation with functional WHILE', () => {
        ...
        const code = 'n := 5; result := 1; i := 1; WHILE(i <= n, (result := result * i; i := i + 1))';
        const tokens = tokenize(code);
        const ast = parse(tokens, systemLoader.createParserLookup());
        ...
        const whileStmt = ast.find(stmt => 
            (stmt.type === 'FunctionCall' && stmt.function?.name === 'WHILE') ||
            (stmt.type === 'Statement' && stmt.expression?.type === 'FunctionCall' && stmt.expression.function?.name === 'WHILE')
        );
        
        // Get the actual function call node
        const actualWhileCall = whileStmt.type === 'Statement' ? whileStmt.expression : whileStmt;
