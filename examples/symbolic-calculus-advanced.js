...
function analyzeNode(node, depth) {
    ...
    switch (node.type) {
        case 'Derivative':
            ...
            if (node.function.type === 'Derivative' || node.function.type === 'Integral') {
...
analyzeCalculusExpression(
    'Alternating calculus operations',
    "'f'('g')"
);

analyzeCalculusExpression(
    'Nested function calculus',
    "SIN(COS(x))''"
);

// Path and parametric derivatives
analyzeCalculusExpression(
    'Path derivative with parameter',
    "f'(r'(t))"
);

analyzeCalculusExpression(
    'Multiple path derivatives',
    "g'(x'(t), y'(t))"
);
...
analyzeCalculusExpression(
    'Mixed partial with integration',
    "'f''[x, y](a, b)"
);
